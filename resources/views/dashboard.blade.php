...
<div class="container py-4">
	<h1 class="text-center mb-4 fw-bold" style="color: #1a365d;">Panel de Administración</h1> @if(session('success'))
 ...
 <i class="bi bi-trash3-fill"></i>
				...
				<div class="card-footer bg-white d-flex justify-content-between align-items-center">
					<div class="text-muted small">
						Mostrando <span id="contadorUsuariosVisibles">{{ count($usuarios) }}</span> de {{ count($usuarios) }} registros
					</div>
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover align-middle mb-0" id="table-control">
							...
							<tbody class="text-center">
								...
								<tr data-plan-recreacional="{{ $formacion->plan_recreacional ?? 'NO' }}">
									<td>{{ $formacion->id }}</td>
									<td>
 ...
 {{ $formacion->ESTADO }}
				...
				<div class="card-footer bg-white d-flex justify-content-between align-items-center">
					<div class="text-muted small">
						Mostrando <span id="contadorControlEstudios">{{ count($control_estudio) }}</span> de {{ count($control_estudio) }} registros
					</div>
				</div>
			</div>
		</div>
						...
						<table class="table table-striped table-hover align-middle mb-0" id="tablaCertificados">
							...
							<tbody>
								...
								<tr data-tipo="{{ $inscripcion->tipo_formacion }}" data-estado="{{ $inscripcion->estado_formacion }}">
									...
									<td>
										@if($inscripcion->tipo_formacion === 'C') {{ $inscripcion->curso }} @elseif($inscripcion->tipo_formacion === 'T') {{ $inscripcion->taller }} @elseif($inscripcion->tipo_formacion === 'D') {{ $inscripcion->diplomado }} @endif
									</td>
									<td class="text-center">
										@switch($inscripcion->tipo_formacion) @case('C')
 ...
 </span> @elseif($inscripcion->estado_formacion === 'pendiente')
										<span class="badge rounded-pill bg-warning bg-opacity-10 text-warning">
 ...
 <i class="bi bi-x-circle me-1"></i> Rechazado
									...
									<td class="text-center">
										@if($inscripcion->estado_formacion === 'pendiente_admin')
        ...
        <i class="bi bi-x-lg"></i> Rechazar
										...
										@elseif($inscripcion->estado_formacion === 'aprobado')
										<span class="badge bg-success">Aprobado</span> @elseif($inscripcion->estado_formacion === 'rechazado_admin')
										<div>
											<span class="badge bg-danger">Rechazado</span> @if($inscripcion->comentarios_rechazo_admin)
										...
										<div>
											<span class="badge bg-warning">Rechazado por facilitador</span> @if($inscripcion->comentarios_rechazo_facilitador)
											<div class="small text-muted mt-1">
											...
											<div class="d-flex justify-content-center gap-2 mt-2">
												<form method="POST" action="{{ route('admin.certificados.aprobar', $inscripcion->id) }}" class="m-0">
													@csrf @method('PUT')
													<button type="submit" class="btn btn-sm btn-success">
                    <i class="bi bi-check-lg"></i> Aprobar igual
                </button>
												</form>
		...
		<div class="card-footer bg-white d-flex justify-content-between align-items-center">
			<div class="text-muted small">
				Mostrando <span id="contadorFilas">{{ count($inscripciones) }}</span> de {{ count($inscripciones) }} registros
			</div>
...
<style>
	/* Estilos generales */
	 ...
	 #tablaCertificados tbody tr, #table-usuarios tbody tr {
	 transition: all 0.2s ease;
	 }
	 
	 #tablaCertificados tbody tr:hover, #table-usuarios tbody tr:hover {
	 ...
	 #filtroRol, #filtroTipo, #filtroEstado, #filtroPlanRecreacional {
	 min-width: 160px;
	 }
	 
	 #buscadorUsuarios, #buscadorGeneral, #buscadorControlEstudios {
	 min-width: 200px;
	 }
	 ...
	 #filtroRol, #filtroTipo, #filtroEstado, 
	 #filtroPlanRecreacional, #buscadorUsuarios, 
	 #buscadorGeneral, #buscadorControlEstudios {
	 width: 100%;
	 }
	 }
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
	 ...
	 fetch(`/admin/certificados/${certificadoId}/detalle`)
	...
	if (rechazarModal) {
	    rechazarModal.addEventListener('show.bs.modal', function(event) {
	        ...
	        const ruta = esAdmin
	            ? `/admin/certificados/${inscripcionId}/rechazar`
	            : `/facilitador/inscripciones/${inscripcionId}/rechazar`;
